...
if (isset($_POST['username']) and isset($_POST['password'])) {
  ...
  if (!$eror) {
    ...
    if ($user) {
        // for update cokie
       $views_count = $_COOKIE['daily_views'] + 1  ; 
      setcookie('daily_views' , $views_count , strtotime('tomorrow') , '/') ;
  ...
  if (isset($_POST['remmember_me'])) {
    ...
    // کلید رمزنگاری AES (باید بطور محرمانه نگهداری شود)
    
    $key = ']vnA5fNxnZ($E5 9ea+:*}O#s.}Z1}Q_B=?2#Jmw`/y92>cEBs0o.#D)A;ef(<v3';

    $enc_user_id =  encrypt( $user_id , $key )   ;


    setcookie('user_id', $enc_user_id, time() + 60 * 60, '/');
